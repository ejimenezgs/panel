(() => {
  ...
  function categoryLabel(value){ return ({poltronas:'Poltronas',sillas:'Sillas',mesas:'Mesas',sofas:'Sofás',decoracion:'Decoración',iluminacion:'Iluminación'})[value] || 'Sin categoría'; }
  ...
  function renderStats(){ const views=state.products.map(productView); $('#stat-total').textContent=views.length; $('#stat-published').textContent=views.filter(p=>p.published).length; $('#stat-stock').textContent=views.filter(p=>p.stock>0).length; $('#stat-no-price').textContent=views.filter(p=>p.price===null).length; }
  ...
  function renderTable(){ const body=$('#products-body'); $('#empty-state').hidden=state.filtered.length>0; body.innerHTML=state.filtered.map(p=>`<tr><td><div class="product-cell"><img class="product-thumb" src="${esc(p.images[0]||FALLBACK_IMAGE)}" alt="" onerror="this.src='${FALLBACK_IMAGE}'"><div><strong>${esc(p.displayName)}</strong><small>${esc(p.description||'Sin descripción')}</small></div></div></td><td>${esc(p.code)}</td><td><span class="category-pill">${esc(categoryLabel(p.category))}</span></td><td>${esc(money(p.price))}</td><td><span class="stock-pill ${p.stock>0?'has-stock':'no-stock'}">${p.stock>0?`${p.stock} disponibles`:'Sin stock'}</span></td><td><label class="visibility-toggle"><input type="checkbox" data-toggle-visible="${esc(p.id)}" ${p.published?'checked':''}><i></i></label></td><td><button class="row-action" type="button" data-edit="${esc(p.id)}" aria-label="Editar ${esc(p.name)}"><i data-lucide="more-horizontal"></i></button></td></tr>`).join(''); renderLucide(); }
  function openDrawer(id){ const product=state.products.find(p=>p.id===id); if(!product)return; const p=productView(product); state.current=product; $('#drawer-title').textContent=p.displayName; $('#drawer-code').textContent=p.code; $('#drawer-price').textContent=money(p.price); $('#drawer-stock').textContent=p.stock>0?`${p.stock} disponibles`:'Sin stock'; $('#drawer-image').src=p.images[0]||FALLBACK_IMAGE; $('#drawer-image').alt=p.displayName; $('#field-published').checked=p.published; $('#field-category-api').value=product.apiCategory || categoryLabel(product.category); $('#field-display-name').value=(state.overrides[p.id]?.displayName||''); $('#field-description').value=(state.overrides[p.id]?.editorialDescription||''); $('#field-order').value=p.order; $('#field-featured').checked=p.featured; $('#field-slug').value=p.slug; $('#inventory-list').innerHTML=[['Código',p.code],['Nombre',p.name],['Descripción',p.description||'—'],['Categoría API',product.apiCategory || categoryLabel(product.category)],['Marca',p.brand],['Color',p.color],['Materiales',p.materials],['Medidas',p.measures],['Precio',money(p.price)],['Stock',p.stock],['Imágenes',p.images.length]].map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`).join(''); $('#drawer-backdrop').hidden=false; requestAnimationFrame(()=>$('#product-drawer').classList.add('is-open')); $('#product-drawer').setAttribute('aria-hidden','false'); }
