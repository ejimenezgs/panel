(() => {
  ...
  function shopContentPayload(content){
    ...
    // sections es la unica estructura canonica que se persiste. Las copias
    // superiores antiguas se siguen leyendo como fallback, pero nunca se vuelven
    // a escribir ni pueden reemplazar URLs absolutas de assets.casaglick.com.
    return {schemaVersion:6,schema:{version:3,sections:schemaPayload(normalized.definitions)},sectionOrder:[...normalized.sectionOrder],sections:{...normalized.sections}};
  }
  ...
  async function persistSectionOrder(){
    ...
    try{
      await cloud.saveWebContent(state.activeWebSite,{sectionOrder,schemaVersion:6});
