(() => {
  ...
  async function markCurrentMessageUnread(){ const item=state.currentMessage; if(!item)return; try{ await cloud.updateMessageStatus(item.id,'unread'); item.status='unread'; applyMessageFilters(); updateMessagesBadge(); closeMessage(); toast('Mensaje marcado como no leído.'); }catch(error){console.error(error);toast('No se pudo actualizar el mensaje.');} }
  function bind(){
    ...
    $$('.nav-item').forEach(btn=>btn.addEventListener('click',()=>{ $$('.nav-item').forEach(x=>x.classList.toggle('is-active',x===btn)); $$('.view').forEach(v=>v.classList.toggle('is-active',v.id===`view-${btn.dataset.view}`)); if(btn.dataset.view==='orders')loadOrders(); if(btn.dataset.view==='messages')loadMessages(); }));
