(() => {
  ...
  function priceMarkup(product){
    ...
    return `<div class="price-promo"><span class="price-promo__current">${esc(money(product.salePrice))}</span><span class="price-promo__original">${esc(money(product.originalPrice))}</span>${product.discountPercent?`<small>-${esc(product.discountPercent)}%</small>`:''}</div>`;
  }
  ...
  function openDrawer(id){ const product=state.products.find(p=>p.id===id); if(!product)return; const p=productView(product); state.current=product; $('#drawer-title').textContent=p.displayName; $('#drawer-code').textContent=p.code; $('#drawer-price').innerHTML=p.hasPromotion?`${esc(money(p.salePrice))} <del>${esc(money(p.originalPrice))}</del>`:esc(money(p.price)); $('#drawer-stock').textContent=p.stock>0?`${p.stock} disponibles`:'Sin stock'; $('#drawer-image').src=p.images[0]||FALLBACK_IMAGE; $('#drawer-image').alt=p.displayName; $('#field-published').checked=p.published; $('#field-category-api').value=product.apiCategory || categoryLabel(product.category); $('#field-display-name').value=(state.overrides[p.id]?.displayName||''); $('#field-description').value=(state.overrides[p.id]?.editorialDescription||''); $('#field-order').value=p.order; $('#field-featured').checked=p.featured; $('#field-slug').value=p.slug; $('#inventory-list').innerHTML=[['Código',p.code],['Nombre',p.name],['Descripción',p.description||'—'],['Categoría API',product.apiCategory || categoryLabel(product.category)],['Marca',p.brand],['Color',p.color],['Materiales',p.materials],['Medidas',p.measures],['Precio',p.hasPromotion?`${money(p.salePrice)} (antes ${money(p.originalPrice)})`:money(p.price)],['Promoción',p.hasPromotion?(p.discountPercent?`${p.discountPercent}% de descuento`:'Activa'):'No'],['Stock',p.stock],['Imágenes',p.images.length]].map(([k,v])=>`<dt>${esc(k)}</dt><dd>${esc(v)}</dd>`).join(''); $('#drawer-backdrop').hidden=false; requestAnimationFrame(()=>$('#product-drawer').classList.add('is-open')); $('#product-drawer').setAttribute('aria-hidden','false'); }
