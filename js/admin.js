(() => {
  ...
  const state = { products: [], filtered: [], current: null, overrides: {}, settings: { apiUrl: DEFAULT_API, stripeEnabled: false }, orders: [], currentOrder: null, messages: [], filteredMessages: [], currentMessage: null, shopContent: {}, activeWebSite: 'shop' };
  const $ = (s, root=document) => root.querySelector(s);
  const $$ = (s, root=document) => [...root.querySelectorAll(s)];
  const cloud = window.CasaGlickFirestore;
  const SUPER_ADMIN_UID = 'nJIkImK4cDdiXghn8wYecqyw1M03';
  const isSuperAdmin = () => document.body.dataset.userUid === SUPER_ADMIN_UID;
  ...
  function normalizeShopContent(raw={}){
    ...
    definitions.forEach(def=>{
      ...
      sections[def.key]=fillEmptySectionFields({...defaults,...directSection,...nestedSection},defaults);
    });
  ...
  function renderShopContent(){
    ...
    const definitions=orderedDefinitions(state.shopContent.definitions||activeDefinitions(),state.shopContent.sectionOrder);
  ...
  async function loadOrders(){ const body=$('#orders-body'); if(body)body.innerHTML='<tr><td colspan="7">Cargando órdenes…</td></tr>'; try{ state.orders=await cloud.loadOrders(); updateOrdersBadge(); if(body)renderOrders(); }catch(error){ console.error(error); if(body)body.innerHTML='<tr><td colspan="7">No fue posible cargar las órdenes.</td></tr>'; } }
  function startOrdersListener(){
    ...
    cloud.subscribeOrders(
      ...
      ()=>{ const body=$('#orders-body'); if(body&&!state.orders.length)body.innerHTML='<tr><td colspan="7">No fue posible actualizar las órdenes.</td></tr>'; }
    );
  }
  ...
  function orderPaymentState(order,channel){
    if(channel.key!=='stripe')return {key:'whatsapp',label:'WhatsApp'};
  ...
  function orderItemName(item,index){ return item?.name||item?.displayName||item?.title||item?.productName||item?.sku||item?.code||`Producto ${index+1}`; }
  function orderItemQty(item){ return Number(item?.quantity??item?.qty??item?.cantidad??1)||1; }
  function orderItemUnitPrice(item){ const value=item?.unitPrice??item?.price??item?.salePrice??item?.precio??0; return Number(value)||0; }
  function orderCustomerRows(order){
    const c=order.customer&&typeof order.customer==='object'?order.customer:{};
    const address=c.address&&typeof c.address==='object'?c.address:(order.address&&typeof order.address==='object'?order.address:{});
    const rows=[
      ['Nombre',c.name||c.fullName||order.customerName||'—'],
      ['Correo',c.email||order.email||'—'],
      ['Teléfono',c.phone||order.phone||'—'],
      ['Entrega',c.deliveryType||order.deliveryType||'—'],
      ['Dirección',[address.street||address.address||c.street,c.exteriorNumber||address.exteriorNumber||address.number,c.interiorNumber||address.interiorNumber].filter(Boolean).join(' ')||'—'],
      ['Colonia',address.neighborhood||address.colony||c.neighborhood||'—'],
      ['Ciudad',address.city||c.city||'—'],
      ['Estado',address.state||c.state||'—'],
      ['Código postal',address.postalCode||address.zip||c.postalCode||'—'],
      ['Referencias',address.references||c.references||order.references||'—']
    ];
  ...
  function openOrderDetails(id){
    ...
    const channel=orderSaleChannel(order); const payment=orderPaymentState(order,channel); const status=orderDisplayStatus(order,payment,channel);
    $('#order-drawer-folio').textContent=order.folio||order.id;
    $('#order-drawer-date').textContent=formatDate(order.createdAt);
    $('#order-drawer-status').textContent=status;
    $('#order-drawer-payment').textContent=payment.label;
    $('#order-drawer-payment').className=`order-pill order-pill--payment is-${payment.key}`;
    $('#order-drawer-total').textContent=money(order.total??0);
    $('#order-drawer-subtotal').textContent=money(order.subtotal??order.total??0);
  ...
  function closeOrderDetails(){ $('#order-drawer')?.classList.remove('is-open'); $('#order-drawer')?.setAttribute('aria-hidden','true'); setTimeout(()=>{if($('#order-backdrop'))$('#order-backdrop').hidden=true;},350); state.currentOrder=null; }
  function renderOrders(){ const body=$('#orders-body'); const empty=$('#orders-empty'); empty.hidden=state.orders.length>0; body.innerHTML=state.orders.map(order=>{ const itemCount=Array.isArray(order.items)?order.items.reduce((sum,item)=>sum+(Number(item.quantity)||1),0):0; const delivery=order.customer?.deliveryType||order.deliveryType||''; const saleChannel=orderSaleChannel(order); const paymentState=orderPaymentState(order,saleChannel); const displayStatus=orderDisplayStatus(order,paymentState,saleChannel); const deleteAction=isSuperAdmin()?`<button class="context-menu__item context-menu__item--danger" type="button" role="menuitem" data-delete-order="${esc(order.id)}" data-order-folio="${esc(order.folio||order.id)}"><i data-lucide="trash-2"></i><span>Eliminar</span></button>`:''; const actionsMenu=`<div class="context-menu"><button class="row-action context-menu__trigger" type="button" data-order-menu-toggle aria-haspopup="menu" aria-expanded="false" aria-label="Más opciones"><i data-lucide="more-vertical"></i></button><div class="context-menu__panel" role="menu" hidden><button class="context-menu__item" type="button" role="menuitem" data-view-order="${esc(order.id)}"><i data-lucide="eye"></i><span>Detalles</span></button>${deleteAction}</div></div>`; const statuses=['Nueva','Contactado','Cotización enviada','Confirmada','Pagada','En preparación','Entregada','Cancelada','No completado']; return `<tr><td><strong>${esc(order.folio||order.id)}</strong><small>${itemCount} producto${itemCount===1?'':'s'}</small></td><td><strong>${esc(order.customer?.name||'—')}</strong><small>${esc(order.customer?.phone||'')}${delivery?` · ${esc(delivery)}`:''}</small></td><td>${esc(formatDate(order.createdAt))}</td><td>${esc(money(order.total??0))}</td><td><span class="order-pill order-pill--payment is-${esc(paymentState.key)}">${esc(paymentState.label)}</span></td><td><select class="order-status" data-order-status="${esc(order.id)}">${statuses.map(status=>`<option ${status===displayStatus?'selected':''}>${status}</option>`).join('')}</select></td><td><div class="row-actions"><button class="row-action" type="button" data-order-whatsapp="${esc(order.customer?.phone||'')}" data-order-folio="${esc(order.folio||'')}" aria-label="Dar seguimiento por WhatsApp"><i data-lucide="message-circle"></i></button>${actionsMenu}</div></td></tr>`; }).join(''); renderLucide(); }
  ...
  function closeContextMenus(except=null){ document.querySelectorAll('.context-menu.is-open').forEach(menu=>{ if(menu===except)return; menu.classList.remove('is-open'); const trigger=menu.querySelector('.context-menu__trigger'); const panel=menu.querySelector('.context-menu__panel'); if(trigger)trigger.setAttribute('aria-expanded','false'); if(panel)panel.hidden=true; }); }
  function toggleContextMenu(trigger){ const menu=trigger?.closest('.context-menu'); if(!menu)return; const willOpen=!menu.classList.contains('is-open'); closeContextMenus(menu); const panel=menu.querySelector('.context-menu__panel'); menu.classList.toggle('is-open',willOpen); trigger.setAttribute('aria-expanded',String(willOpen)); if(!panel)return; panel.classList.remove('context-menu__panel--up'); panel.hidden=!willOpen; if(willOpen){ const panelHeight=panel.offsetHeight||96; const triggerRect=trigger.getBoundingClientRect(); const scrollContainer=trigger.closest('.table-wrap'); const boundaryBottom=Math.min(window.innerHeight,scrollContainer?.getBoundingClientRect().bottom||window.innerHeight); const roomBelow=boundaryBottom-triggerRect.bottom; const roomAbove=triggerRect.top-Math.max(0,scrollContainer?.getBoundingClientRect().top||0); if(roomBelow<panelHeight+10&&roomAbove>roomBelow){ panel.classList.add('context-menu__panel--up'); } } }
  async function deleteCurrentMessage(){ if(!isSuperAdmin()||!state.currentMessage)return; const item=state.currentMessage; if(!window.confirm(`¿Eliminar permanentemente el mensaje de ${item.name||'este contacto'}?\n\nEsta acción no se puede deshacer.`))return; const button=$('#delete-message'); if(button)button.disabled=true; try{ await cloud.deleteMessage(item.id); closeMessage(); toast('Mensaje eliminado permanentemente.'); }catch(error){ console.error(error); toast('No se pudo eliminar el mensaje.'); }finally{ if(button)button.disabled=false; } }
  async function deleteOrder(id,folio){ if(!isSuperAdmin()||!id)return; if(!window.confirm(`¿Eliminar permanentemente la orden ${folio||id}?\n\nEsta acción no se puede deshacer.`))return; try{ await cloud.deleteOrder(id); toast('Orden eliminada permanentemente.'); }catch(error){ console.error(error); toast('No se pudo eliminar la orden.'); } }
  let draggedSectionKey='';
  let dragArmedKey='';
  function currentSectionOrderFromDom(){
    const definitions=state.shopContent.definitions||activeDefinitions();
  ...
  async function persistSectionOrder(){
    ...
    try{
      await cloud.saveWebContent(state.activeWebSite,{sectionOrder,schemaVersion:5});
  ...
  function bind(){
    ...
    $('#message-source-filter')?.addEventListener('change',applyMessageFilters); $('#message-status-filter')?.addEventListener('change',applyMessageFilters); $('#messages-body')?.addEventListener('click',e=>{ const btn=e.target.closest('[data-open-message]'); const row=e.target.closest('[data-message-row]'); const id=btn?.dataset.openMessage||row?.dataset.messageRow; if(id)openMessage(id); }); $('#close-message-drawer')?.addEventListener('click',closeMessage); $('#message-backdrop')?.addEventListener('click',closeMessage); $('#delete-message')?.addEventListener('click',()=>{ closeContextMenus(); deleteCurrentMessage(); }); $('#message-actions-toggle')?.addEventListener('click',e=>{ e.stopPropagation(); toggleContextMenu(e.currentTarget); });
    $('#orders-body')?.addEventListener('click',e=>{ const menuToggle=e.target.closest('[data-order-menu-toggle]'); if(menuToggle){ e.stopPropagation(); toggleContextMenu(menuToggle); return; } const detailsBtn=e.target.closest('[data-view-order]'); if(detailsBtn){ closeContextMenus(); openOrderDetails(detailsBtn.dataset.viewOrder); return; } const deleteBtn=e.target.closest('[data-delete-order]'); if(deleteBtn){ closeContextMenus(); deleteOrder(deleteBtn.dataset.deleteOrder,deleteBtn.dataset.orderFolio); return; } const btn=e.target.closest('[data-order-whatsapp]'); if(!btn)return; const phone=String(btn.dataset.orderWhatsapp||'').replace(/\D/g,''); const text=encodeURIComponent(`Hola, damos seguimiento a tu orden ${btn.dataset.orderFolio} de Casa Glick.`); window.open(`https://wa.me/${phone}?text=${text}`,'_blank'); });
    $('#close-order-drawer')?.addEventListener('click',closeOrderDetails); $('#order-backdrop')?.addEventListener('click',closeOrderDetails);
    document.addEventListener('click',e=>{ if(!e.target.closest('.context-menu'))closeContextMenus(); }); document.addEventListener('keydown',e=>{ if(e.key==='Escape'){ closeContextMenus(); closeOrderDetails(); } });
    const stripeToggle=$('#stripe-enabled');
    ...
    $('#shop-content-list')?.addEventListener('change',async e=>{ const fileInput=e.target.closest('[data-section-upload]'); if(!fileInput||!fileInput.files?.[0])return; const key=fileInput.dataset.sectionUpload; const field=fileInput.dataset.sectionUploadField||'imageUrl'; const token=`${key}:${field}`; const progress=$(`[data-section-progress="${token}"]`); const file=fileInput.files[0]; if(progress)progress.textContent='Subiendo a GoDaddy…'; fileInput.disabled=true; try{ const url=await cloud.uploadShopContentImage(key,file,state.activeWebSite); await cloud.saveWebContentImage(state.activeWebSite,key,field,url); const content=collectShopContent(); if(!content.sections[key])content.sections[key]={}; content.sections[key][field]=url; state.shopContent=normalizeShopContent(content); const urlInput=$(`[data-section-key="${key}"][data-section-field="${field}"]`); const preview=$(`[data-section-preview="${token}"]`); if(urlInput)urlInput.value=url; if(preview)preview.src=url; if(progress)progress.textContent=`Imagen publicada en ${activeSiteName()}.`; const status=$('#shop-content-status'); if(status){status.textContent='Imagen sincronizada';status.classList.add('is-saved');} toast(`Imagen subida y actualizada en ${activeSiteName()}.`); }catch(error){console.error(error);if(progress)progress.textContent=error?.message||'No se pudo subir la imagen.';toast('No se pudo publicar la imagen.');}finally{fileInput.disabled=false;fileInput.value='';} });
    ...
    $('#reset-shop-content')?.addEventListener('click',()=>{
      if(!window.confirm('¿Restablecer todas las secciones a sus valores predeterminados? Los cambios se aplicarán en Firebase hasta que presiones Guardar cambios.'))return;
      const definitions=state.shopContent.definitions||activeDefinitions();
      const sections={};
      definitions.forEach(def=>{ sections[def.key]=defaultSection(def); });
      state.shopContent={...state.shopContent,sections,sectionOrder:normalizeSectionOrder(state.shopContent.sectionOrder,definitions)};
      renderShopContent();
      const status=$('#shop-content-status');
      if(status){status.textContent='Cambios sin guardar';status.classList.remove('is-saved');}
      toast('Valores predeterminados restaurados. Guarda los cambios para aplicarlos.');
    });
