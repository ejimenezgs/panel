(() => {
  ...
  const state = { products: [], filtered: [], current: null, overrides: {}, settings: { apiUrl: DEFAULT_API, stripeEnabled: false }, orders: [], currentOrder: null, messages: [], filteredMessages: [], currentMessage: null, shopContent: {} };
  ...
  function bind(){
    ...
    $('#shop-content-list')?.addEventListener('change',async e=>{ const fileInput=e.target.closest('[data-section-upload]'); if(!fileInput||!fileInput.files?.[0])return; const key=fileInput.dataset.sectionUpload; const progress=$(`[data-section-progress="${key}"]`); const file=fileInput.files[0]; if(progress)progress.textContent='Subiendo a GoDaddy…'; fileInput.disabled=true; try{ const url=await cloud.uploadShopContentImage(key,file); await cloud.saveShopContentImage(key,url); const urlInput=$(`[data-section-key="${key}"][data-section-field="imageUrl"]`); const preview=$(`[data-section-preview="${key}"]`); if(urlInput)urlInput.value=url; if(preview)preview.src=url; if(state.shopContent.sections?.[key]) state.shopContent.sections[key].imageUrl=url; if(progress)progress.textContent='Imagen publicada en Shop.'; const status=$('#shop-content-status'); if(status){status.textContent='Imagen sincronizada';status.classList.add('is-saved');} toast('Imagen subida a GoDaddy y actualizada en Shop.'); }catch(error){console.error(error);if(progress)progress.textContent=error?.message||'No se pudo subir la imagen.';toast('No se pudo publicar la imagen.');}finally{fileInput.disabled=false;fileInput.value='';} });
